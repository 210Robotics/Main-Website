...
export async function GET(request: Request) {
  ...
  const buffer = await buildEngineeringNotebookDocument(data);
  const filename = `210-Robotics-Engineering-Notebook-${safeScope}-${new Date().toISOString().slice(0, 10)}.docx`;
