...
export function BlogEditor({ name = "bodyHtml", initial = "<p>Share the story behind the work.</p>" }: { name?: string; initial?: string }) {
  ...
  const editor = useEditor({ immediatelyRender: false, extensions: [StarterKit.configure({ link: { openOnClick: false } }), Image], content: initial, onUpdate: ({ editor: current }) => setHtml(current.getHTML()), editorProps: { attributes: { class: "min-h-56 p-4 outline-none prose-editor" } } });
